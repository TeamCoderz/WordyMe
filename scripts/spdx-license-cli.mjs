...
async function getRepositoryFiles() {
  ...
  return Buffer.concat(chunks).toString('utf8').split('\u0000').filter(Boolean);
}

async function main() {
  const candidateFiles = fileArgs.length > 0 ? fileArgs : await getRepositoryFiles();
  const normalizedTargets = Array.from(
    new Set(candidateFiles.map(normalizeRelativePath).filter(isRelevantFile)),
  );
  ...
  const ignoreContents =
    ['*', ...normalizedTargets.map((target) => `!${target}`)].join('\n') + '\n';
