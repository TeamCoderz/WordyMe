...
export const updateFontSizeInSelection = (
  ...
  editor.update(
    () => {
      if (editor.isEditable()) {
        ...
        if (selection !== null) {
          ...
          const nodes = selection.getNodes().filter((node) => !$isTextNode(node));
          $patchStyle(nodes, {
    ...
    { tag: skipDomSelection ? 'skip-dom-selection' : undefined },
  );
};
...
export const updateFontSize = (
  ...
  if (inputValue !== '') {
    ...
    updateFontSizeInSelection(editor, String(nextFontSize) + 'px', null, skipDomSelection);
  } else {
    updateFontSizeInSelection(editor, null, updateType, skipDomSelection);
  }
};
