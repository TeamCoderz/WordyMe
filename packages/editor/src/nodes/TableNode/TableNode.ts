...
import { floatWrapperElement } from '@repo/editor/utils/nodeUtils';
...
export class TableNode extends LexicalTableNode {
  ...
  createDOM(config: EditorConfig, editor: LexicalEditor): HTMLElement {
    ...
    wrapperElement.setAttribute('style', `anchor-name: --table-anchor-${this.getKey()};`);
    const float = getStyleObjectFromCSS(this.__style).float;
...
export function $convertTableElement(_domNode: Node): DOMConversionOutput {
  ...
  const format = (domNode.style.textAlign as ElementFormatType) || 'justify';
  tableNode.setFormat(format);
  const { color, backgroundColor, float } = domNode.style;
  const cssText = getCSSFromStyleObject({ color, backgroundColor, float });
  tableNode.__style = cssText;
