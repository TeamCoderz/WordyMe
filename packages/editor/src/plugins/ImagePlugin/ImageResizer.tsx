...
export function ImageResizerPlugin() {
  ...
  useEffect(() => {
    ...
    editor.read(() => {
      ...
      if ($isImageNode(selectedNode)) {
        const figure = editor.getElementByKey(selectedNode.getKey());
        if (figure) {
          const image = figure.querySelector('img, iframe') as
            | HTMLImageElement
            | HTMLIFrameElement
            | null;
          ...
          if (image && controlWrapper) {
            controlWrapper.setAttribute(
              'style',
              `position-anchor: --image-anchor-${selectedNode.getKey()}; top: anchor(top); right: anchor(right); bottom: anchor(bottom); left: anchor(left);`,
            );
            anchorPolyfill([image, controlWrapper]);
          }
        }
      } else {
        imageRef.current = null;
        if (controlWrapper) controlWrapper.removeAttribute('style');
      }
    });
  }, [editor, selectedNode]);
  ...
  const onResizeEnd = (nextWidth: number, nextHeight: number) => {
    ...
    editor.update(() => {
      ...
      selectedNode.setWidthAndHeight(nextWidth, nextHeight);
    });
  };
  ...
  const maxWidthContainer =
    editorRootElement !== null ? editorRootElement.getBoundingClientRect().width - 20 : 40;
  const maxHeightContainer =
    editorRootElement !== null ? editorRootElement.getBoundingClientRect().height - 20 : 40;
  ...
  const handlePointerMove = (event: PointerEvent) => {
    ...
    if (image !== null && positioning.isResizing && controlWrapper !== null) {
      const zoom = calculateZoomLevel(image);
  ...
  const showResizers = isImageNode || isResizing;
  ...
  return (
    <div
      ...
      className={cn('image-control-wrapper', {
        'image-control-wrapper--active': isImageNode,
        'image-control-wrapper--resizing': isResizing,
      })}
      onClick={() => {
        restoreFocus(editor);
      }}
