...
import { $isHeadingNode, HeadingNode } from '@repo/editor/nodes/HeadingNode';
...
function toEntry(heading: HeadingNode): TableOfContentsEntry {
  const id = heading.getId() || formatId(heading.getTextContent());
  return [heading.getKey(), heading.getTextContent(), heading.getTag(), id];
}
    ...
    const removeRootUpdateListener = editor.registerUpdateListener(
      ({ editorState, dirtyElements }) => {
        editorState.read(() => {
          ...
          // If only the root node is changed, update the table of contents
          if (dirtyElements.size === 1) {
            currentTableOfContents = [];
            updateCurrentTableOfContents($getRoot());
            setTableOfContents([...currentTableOfContents]);
          } else {
