...
export function SchemaToolbar({
  ...
  const slotClasses = isSecondarySlot ? '' : '';

  return (
    <div
      className={cn('flex items-center gap-2', placementClasses, slotClasses, className)}
...
function TabGroupRenderer({
  ...
  return (
    <div
      className={cn('flex items-center', alignmentClass)}
      {...getUIItemProps(item)}
      role="tablist"
    >
...
function GroupRenderer({
  ...
  return (
    <div className={cn('flex items-center', gapClass, alignmentClass)} {...getUIItemProps(item)}>
...
function getPlacementClasses(placement: 'top' | 'bottom' | 'left' | 'right'): string {
  switch (placement) {
    case 'top':
      return 'border-b px-3 py-2';
    case 'bottom':
      return 'border-t px-3 py-2';
    case 'left':
      return 'border-r px-2 py-3 flex-col';
    case 'right':
      return 'border-l px-2 py-3 flex-col';
  }
}
