...
export function SchemaMenu({ schema, documentId, anchorEl, onClose }: MenuRendererProps) {
  ...
  if (isMobile) {
    return (
      <>
        ...
        <div
          ref={menuRef}
          className="animate-slide-up fixed bottom-0 left-0 right-0 z-50 max-h-[80vh] overflow-y-auto rounded-t-2xl bg-background shadow-2xl"
  ...
  return (
    <div
      ref={menuRef}
      className="animate-fade-in fixed z-50 min-w-[200px] rounded-lg border bg-background shadow-lg"
    ...
    >
      {/* Header for submenus */}
      {menuStack.length > 1 && (
        <div className="flex items-center rounded-t-lg border-b border-border bg-background px-2 py-2">
          <button
            onClick={navigateBack}
            className="flex items-center gap-1 text-sm font-medium text-accent hover:text-accent-foreground"
            aria-label="Go back"
          >
    ...
    case 'divider':
      return (
        <div {...getUIItemProps(item)}>
          <div
            className={isMobile ? 'my-2 border-t border-border' : 'my-1 border-t border-border'}
          />
        </div>
      );
...
function CommandMenuItem({
  ...
  const baseClasses = isMobile
    ? 'flex items-center gap-3 px-4 py-3 text-base transition-colors active:bg-accent/75 hover:text-accent-foreground'
    : 'flex items-center gap-2 px-3 py-2 text-sm transition-colors hover:bg-accent/75 hover:text-accent-foreground';
  ...
  return (
    <button
      ...
      className={cn(baseClasses, disabledClasses, activeClasses, 'w-full text-left')}
      role="menuitem"
    >
      {IconComponent && (
        <IconComponent
      ...
      {command.shortcuts && command.shortcuts.length > 0 && !isMobile && (
        <span className="text-xs text-muted-foreground">{command.shortcuts[0]}</span>
      )}
    </button>
  );
}
...
function SubmenuItem({
  ...
  return (
    <button
      {...getUIItemProps(item)}
      onClick={handleClick}
      className={cn(baseClasses, 'w-full cursor-pointer text-left text-muted-foreground')}
      role="menuitem"
    >
